...
// Rate limit constants for Gemini
const RATE_LIMITS = {
  'gemini-3-pro': {
...
// POST /api/scripts/generate-gemini
        ...
        try {
          ...
          while (currentSectionNumber <= maxSections) {
            ...
            const generationSettings: ChatSettings = {
              ...settings,
              model: settings.model || 'gemini-3-pro',
            };

            // Stream the section generation with retry logic
            try {
              await retryWithBackoff(async () => {
                for await (const chunk of geminiAPI.streamMessage(messages, generationSettings, channel.systemPrompt)) {
            ...
            // Wait 2 seconds between sections to avoid rate limits
            if (currentSectionNumber <= maxSections) {
              sendEvent('waiting', {
                message: 'Waiting 2 seconds to avoid rate limits...',
                nextSection: currentSectionNumber,
              });
              await waitBetweenSections(2);
            }
          }
